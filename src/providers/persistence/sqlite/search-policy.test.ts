...
import type { SearchIntent, SearchMode } from './search-policy'
import {
    allowResult,
...
type CoveredTypes = [SearchIntent, SearchMode]
...
describe('providers/persistence/sqlite/search-policy', () => {
    ...
    it('compiles search policy type exports', () => {
        type _Covered = CoveredTypes
        const names = ['SearchIntent', 'SearchMode'] as const
        expect(names).toEqual(['SearchIntent', 'SearchMode'])
    })
})
